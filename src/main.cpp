...
void init(void)
{
	...
	MeshObj test_obj;
	meshCreate(&attrib, &shapes, &materials, &test_obj, program);
	meshes.push_back(test_obj);

	uniform_index = glGetUniformBlockIndex(program, "eye_and_light");

	const char *uniform_names[NumUniformLocation] = {"light_color", "eye_direction", "light_pos"};
